public class DoublyLinkedList {
    ...
    public void insertLast(int data){
        ...
        if(isEmpty()){
            ...
            newNode.previous = last; // old last will be the neww node's previous
        }
    ...
    public Node deleteFirst(){
        Node temp = first;
        if(first.next == null){   // assume there is only one item in the list
            last = null;
        }else{
            first.next.previous = null; // the list's first node will point to null
        }
        first = first.next; // assigning the refference of the node following the old first node to the first field in the linkedlist object
    ...
    public Node deleteLast(){
        Node temp = last;
        if(first.next == null){   // assume there is only one node  in the list
            first = null;
        }else{
            last.previous.next = null; // the list's last node 's previous node's next field will point to null
        }
        last = last.previous; // assigning the refference of the node following the old last node to the last field in the linkedlist object
    ...
    public boolean inserAfter(int key, int data){
        ...
        while(current.data != key){ //as long as we have not fount the key in a particular node
